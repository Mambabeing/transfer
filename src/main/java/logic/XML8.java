...
//    }
    ...
    public static void updateAttribute(Document document)throws Exception{
        ...
        for(Element e:students){
            e.addAttribute("班级编号","C2");//增加结点属性
        }
        updateToXML(document);
    }
    public static void updateElement(Document document)throws Exception{
        List<Element> students=document.getRootElement().elements("学生");//展示所有学生
        for(Element e:students){
            Element age=e.element("年龄");
            age.setText(Integer.parseInt(age.getTextTrim())+3+"");//设置结点内容
        }
        updateToXML(document);
    }
    ...
    public static void add(Document document)throws Exception{
        Element newStu=DocumentHelper.createElement("学生");  //创建结点
        newStu.addAttribute("学号","n4");                         //设置结点属性
        Element newStuName=DocumentHelper.createElement("名字");//结点属性1
        Element newStuAge=DocumentHelper.createElement("年龄");//结点属性2
        Element newStuIntro=DocumentHelper.createElement("介绍");//结点属性3
